 ...
 * @author krogulecp
 */
class UnitedKingdomSalaryPlnCalculatorTest {

    private static final String EXCHANGE_RATE_URL = "/exchangerates/rates/a/";
    private static final BigDecimal EXPECTED_RATE = new BigDecimal(6.00);
    private static final String EXPECTED_RETURN = "{\"table\":\"A\",\"currency\":\"funt\",\"code\":\"GBP\",\"rates\":[{\"no\":\"129/A/NBP/2019\",\"effectiveDate\":\"2019-07-05\",\"mid\":" + EXPECTED_RATE.doubleValue() + "}]}";

    private RestTemplate restTemplate;
    private UnitedKingdomSalaryCalculator unitedKingdomSalaryCalculator;
    private ToPlnConverter toPlnConverter;
    private UnitedKingdomConfigurationProperties ukProps;
    ...
    void setUpBeforeEach(){
        ...
        ukProps = new UnitedKingdomConfigurationProperties();
    ...
    void should_calculate_salary(BigDecimal dailyRate, double taxPercentage, BigDecimal fixedCosts, BigDecimal expectedSalaryPln){
        ...
        ukProps = new UnitedKingdomConfigurationProperties();
        ukProps.setFixedCosts(fixedCosts);
        ukProps.setTaxPercentage(taxPercentage);
        ...
        unitedKingdomSalaryCalculator = new UnitedKingdomSalaryCalculator(ukProps, toPlnConverter);

        mockServer.expect(requestTo(EXCHANGE_RATE_URL + Country.UNITED_KINGDOM.getCurrencyCode())).andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess(EXPECTED_RETURN, MediaType.APPLICATION_JSON));

        //when
        SalaryPln salaryPln = unitedKingdomSalaryCalculator.calculateSalary(dailyRate);
    ...
    @Test
    void should_return_uk(){
        unitedKingdomSalaryCalculator = new UnitedKingdomSalaryCalculator(ukProps, toPlnConverter);
        assertThat(unitedKingdomSalaryCalculator.getCountry()).isEqualTo(Country.UNITED_KINGDOM);
    }
